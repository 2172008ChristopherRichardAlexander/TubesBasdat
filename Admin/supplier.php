...
<div class="container">
<form method="POST" id="form1" action=<?php echo "supplier.php?q=".$_SESSION['userLogin']?>>
<table class="table table-striped table-dark table-bordered table-hover">
    ...
    <tbody>
        ...
        while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
            ...
            echo "<td> <input type='hidden' name='vendor-".$num."' value='".$row['VendorName']."'>". $row['VendorName']; " </td>";
            echo "<td> <div> <button type='button' onclick='aa()' value='".$num."' class='btn btn-success'>Update</button> <button type='submit' value='".$row['VendorCode']."' name='delete' class='btn btn-danger'>Delete</button> </div> </td>";
            echo "</tr>";
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <!-- FORMS -->
    <form class="form-horizontal" method="POST" action=<?php echo "supplier.php?q=".$_SESSION['userLogin'] ?>>
        <div class="form-group">
            <label for="VendorName" class="col-sm-2 control-label">VendorName</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="VendorName" name="VendorName" placeholder="VendorName">
                </div>
        </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <form class="form-horizontal" method="POST" action=<?php echo "supplier.php?q=".$_SESSION['userLogin'] ?>>
        <div class="form-group">
            <label for="VendorName" class="col-sm-2 control-label">VendorName</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="VendorName" name="VendorName" placeholder="VendorName" value="">
                </div>
        </div>
      </div>
      <div class="modal-footer">
...
function tambah() {
    ...
    $sql = "EXEC [dbo].[SP_AutoSupp] '". $vendor. "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
...
function delete() {
    include ('../connect.php');
    $pk = $_POST['delete'];
    $sql = "DELETE FROM tbSupplier WHERE VendorCode = '". $pk . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
