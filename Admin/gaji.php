...
<div class="container">
<form method="POST" action=<?php echo "gaji.php?q=".$_SESSION['userLogin'] ?>>
<table class="table table-striped table-dark table-bordered table-hover">
    <thead class="thead-dark">
        <tr>
            <th scope="col">#</th>
            <th scope="col">IdGaji</th>
            <th scope="col">NoIndukPegawai</th>
            <th scope="col">TotalGaji</th>
    ...
    <tbody>
        ...
        while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
            echo "<tr>";
            echo "<td>". $num; "</td>";
            echo "<td>". $row['IdGaji']; "</td>";
            echo "<td>". $row['NoIndukPegawai']; "</td>";
            echo "<td>". $row['TotalGaji']; "</td>";
            echo "<td>". $row['Cuti']; "</td>";
            echo "<td> <div> <button type='submit' class='btn btn-success'>Update</button> <button type='submit' value='".$row['IdGaji']."' name='delete' class='btn btn-danger'>Delete</button> </div> </td>";
            echo "</tr>";
            $num = $num + 1;
        }   
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <!-- FORMS -->
    <form class="form-horizontal" method="POST" action=<?php echo "gaji.php?q=".$_SESSION['userLogin'] ?>>
        <div class="form-group">
            <label for="NoIndukPegawai" class="col-sm-3 control-label">NoIndukPegawai</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="NoIndukPegawai" name="NoIndukPegawai" placeholder="NoIndukPegawai">
                </div>
        </div>
        <div class="form-group">
            <label for="TotalGaji" class="col-sm-2 control-label">TotalGaji</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="TotalGaji" name="TotalGaji" placeholder="TotalGaji">
                </div>
        </div>
        <div class="form-group">
            <label for="Cuti" class="col-sm-2 control-label">Cuti</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="Cuti" name="Cuti" placeholder="Cuti">
                </div>
        </div>
      </div>
...
function tambah() {
    include ('../connect.php');
    $noind = $_POST['NoIndukPegawai'];
    $gaji = $_POST['TotalGaji'];
    $Cuti = $_POST['Cuti'];
    $sql = "EXEC [dbo].[SP_AutoGaji] '". $noind. "', '". $gaji . "', ". $Cuti ;
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
...
function delete() {
    include ('../connect.php');
    $pk = $_POST['delete'];
    $sql = "DELETE FROM tbGaji WHERE IdGaji = '". $pk . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
