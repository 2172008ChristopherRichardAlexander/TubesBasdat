                ...
                <div class="col-lg-12">
                    ...
                    <div class="navbar-collapse collapse" id="mobile_menu">
                        <ul class="nav navbar-nav">
                            ...
                            <li class="active" ><a href="#" class="dropdown-toggle" data-toggle="dropdown"> Pabrik <span class="caret"></span></a>
                                <ul class="dropdown-menu">
...
<div class="container">
<form method="POST" action=<?php echo "cat.php?q=".$_SESSION['userLogin'] ?>>
<table class="table table-striped table-dark table-bordered table-hover">
    <thead class="thead-dark">
        <tr>
            ...
            <th scope="col">KodeProduct</th>
            <th scope="col">ColorCode</th>
            <th scope="col">TipeBarang</th>
    ...
    <tbody>
        ...
        while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
            ...
            echo "<td>". $row['KodeProduct']; "</td>";
            echo "<td>". $row['ColorCode']; "</td>";
            echo "<td>". $row['TipeBarang']; "</td>";
            echo "<td> <div> <button type='submit' class='btn btn-success'>Update</button> <button type='submit' value='".$row['ProductNickName']."' name='delete' class='btn btn-danger'>Delete</button> </div> </td>";
            echo "</tr>";
            $num = $num + 1;
        }   
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <!-- FORMS -->
    <form class="form-horizontal" method="POST" action=<?php echo "cat.php?q=".$_SESSION['userLogin'] ?>>
        ...
        <div class="form-group">
            <label for="KodeProduct" class="col-sm-2 control-label">KodeProduct</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="KodeProduct" name="KodeProduct" placeholder="KodeProduct">
                </div>
        </div>
        <div class="form-group">
            <label for="ColorCode" class="col-sm-2 control-label">ColorCode</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="ColorCode" name="ColorCode" placeholder="ColorCode">
                </div>
        </div>
        <div class="form-group">
            <label for="TipeBarang" class="col-sm-2 control-label">TipeBarang</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="TipeBarang" name="TipeBarang" placeholder="TipeBarang">
                </div>
        </div>
      </div>
...
function tambah() {
    ...
    $kdproduct = $_POST['KodeProduct'];
    $colorcode = $_POST['ColorCode'];
    $tipe = $_POST['TipeBarang'];

    $sql = "EXEC [dbo].[SP_AutoCat] '". $kdpabrik. "', '" . $brandname . "', '" . $kdproduct . "', '" . $colorcode . "', '" . $tipe . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
...
function delete() {
    include ('../connect.php');
    $pk = $_POST['delete'];
    $sql = "DELETE FROM tbCat WHERE ProductNickName = '". $pk . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
