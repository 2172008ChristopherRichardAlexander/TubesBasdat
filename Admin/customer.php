...
<div class="container">
<form method="POST" action=<?php echo "customer.php?q=".$_SESSION['userLogin'] ?>>
<table class="table table-striped table-dark table-bordered table-hover">
    <thead class="thead-dark">
        <tr>
            <th scope="col">#</th>
            <th scope="col">IdAkun</th>
            <th scope="col">FirstName</th>
            <th scope="col">LastName</th>
            <th scope="col">Username</th>
            <th scope="col">PhoneNumber</th>
            <th scope="col">Password</th>
            <th scope="col">TipeAkun</th>
    ...
    <tbody>
        ...
        while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
            echo "<tr>";
            echo "<td>". $num; "</td>";
            echo "<td>". $row['IdAkun']; "</td>";
            echo "<td>". $row['FirstName']; "</td>";
            echo "<td>". $row['LastName']; "</td>";
            echo "<td>". $row['Username']; "</td>";
            echo "<td>". $row['PhoneNumber']; "</td>";
            echo "<td>". $row['Password']; "</td>";
            echo "<td>". $row['TipeAkun']; "</td>";
            echo "<td> <div> <button type='submit' class='btn btn-success'>Update</button> <button type='submit' value='".$row['IdAkun']."' name='delete' class='btn btn-danger'>Delete</button> </div> </td>";
            echo "</tr>";
            $num = $num + 1;
        }   
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <!-- FORMS -->
    <form class="form-horizontal" method="POST" action=<?php echo "customer.php?q=".$_SESSION['userLogin'] ?>>
        ...
        <div class="form-group">
            <label for="TipeAkun" class="col-sm-2 control-label">TipeAkun</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="TipeAkun" name="TipeAkun" placeholder="TipeAkun">
                </div>
        </div>
      </div>
...
function tambah() {
    ...
    $pass = $_POST['Password'];
    $tipe = $_POST['TipeAkun'];
    $sql = "EXEC [dbo].[SP_AutoAkun] '". $fname. "', '". $lname . "', '". $username . "', '". $pnumber . "', '" . $pass . "', '". $tipe . "'" ;
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
...
function delete() {
    include ('../connect.php');
    $pk = $_POST['delete'];
    $sql = "DELETE FROM tbAkun WHERE IdAkun = '". $pk . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
