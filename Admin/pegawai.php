...
<div class="container">
<form method="POST" action=<?php echo "pegawai.php?q=".$_SESSION['userLogin'] ?>>
<table class="table table-striped table-dark table-bordered table-hover">
    ...
    <tbody>
        ...
        while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
            ...
            echo "<td>". $row['Nama']; "</td>";
            echo "<td>". $row['TanggalLahir']->format('d-m-Y H:i:s'); "</td>";
            echo "<td>". $row['Alamat']; "</td>";
            echo "<td>". $row['NoTelpon']; "</td>";
            echo "<td> <div> <button type='submit' class='btn btn-success'>Update</button> <button type='submit' value='".$row['NoIndukPegawai']."' name='delete' class='btn btn-danger'>Delete</button> </div> </td>";
            echo "</tr>";
            $num = $num + 1;
        }   
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <!-- FORMS -->
    <form class="form-horizontal" method="POST" action=<?php echo "pegawai.php?q=".$_SESSION['userLogin'] ?>>
        ...
        <div class="form-group">
            <label for="TanggalLahir" class="col-sm-2 control-label">TanggalLahir</label>
                <div class="col-sm-10">
                    <input type="date" class="form-control" id="TanggalLahir" name="TanggalLahir" placeholder="TanggalLahir">
                </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="NoTelpon" class="col-sm-2 control-label">NoTelpon</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="NoTelpon" name="NoTelpon" placeholder="NoTelpon">
                </div>
        </div>
      </div>
...
function tambah() {
    ...
    $sql = "EXEC [dbo].[SP_AutoPegawai] '". $kdpabrik. "', '". $nama . "', '". $tgl . "', '" . $alamat . "', '". $notelp . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
...
function delete() {
    include ('../connect.php');
    $pk = $_POST['delete'];
    $sql = "DELETE FROM tbPegawai WHERE NoIndukPegawai = '". $pk . "'";
    $result = sqlsrv_query($conn, $sql);
    if ($result) {
